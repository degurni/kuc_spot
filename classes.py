...
class Bot:
    ...
    def create_order(self, symbol, side):
        ...
        elif inf_order['side'] == 'sell':  # Если ордер на продажу
            for i in res:  # Высчитываем прибыль
                if i['symbol'] == symbol:
                    i['result'] += float(inf_order['dealFunds'])
                    i['result'] -= float(inf_order['fee'])
                    break
            data.pop(-1)  # Удаляем запись об ордере из файла
            if len(data) == 0:  # Если это был последний ордер
                p = True
                for i in res:
                    if i['symbol'] == symbol and not conf.sum_result:
                        i['result'] = 0
                        break
            else:
